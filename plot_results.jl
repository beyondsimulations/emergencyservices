...
## Group the results for evaluation
    weekly_location = groupby(dropmissing(select!(copy(incidents), Not([:priority]))), [:location_responsible, :weekhour])
    ...
    weekly_location[!,:exchange_ratio] = 1 .- weekly_location[!,:cars_location_responsible] ./ weekly_location[!,:dispatched_cars]
    weekly_priority = groupby(dropmissing(select!(copy(incidents), Not([:location_responsible]))), [:weekhour, :priority])
    ...
    weekly_priority[!,:exchange_ratio] = 1 .- weekly_priority[!,:cars_location_responsible] ./ weekly_priority[!,:dispatched_cars]
    overall_skipmissing = combine(incidents,
                        ...
                        :cars_missing => mean => :cars_missing,
                        :cars_location_responsible => mean => :cars_location_responsible)
    overall_skipmissing[!,:incidents_unfulfilled] = size(incidents,1) .- overall_skipmissing[!,:total_incidents_fulfilled]
    overall_missing = combine(dropmissing(select!(copy(incidents), Not([:arrival_minute_all,:dispatch_minute_all,:response_time_all,:dispatch_time_all]))),
                        nrow => :total_incidents_fulfilled,
                        :cars => mean => :requested_cars,
                        :cars_dispatched => mean => :dispatched_cars,
                        :dispatch_time_first => mean => :dispatch_time_first,
                        ...
                        :cars_missing => mean => :cars_missing,
                        :cars_location_responsible => mean => :cars_location_responsible)
    overall_missing[!,:incidents_unfulfilled] = size(incidents,1) .- overall_missing[!,:total_incidents_fulfilled]
    overall_missing[!,:cars_total_dispatched] = overall_missing[!,:dispatched_cars] .* overall_missing[!,:total_incidents_fulfilled]
